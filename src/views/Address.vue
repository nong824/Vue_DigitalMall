<template>
    <div>
      ...
      <div class="checkout-page">
        ...
        <div class="container">
          ...
          <div class="md-modal-inner">
            <div class="md-top">
              <div class="md-title">添加新的地址</div>
              <button class="md-close" @click="addAddressFlag=false">Close</button>
            </div>
            <div class="md-content">
              <div class="confirm-tips">
                <div class="error-wrap">
                  <span class="error error-show " v-show="errorTipsDisplay">{{errorTips}}</span>
                  <span class="rightTip error-show " v-show="rightTipsDisplay">{{rightTips}}</span>
                </div>
                <ul>
                  <li class="regi_form_input">
                    <i class="icon IconPeople"></i>
                    <input type="text" tabindex="1"  class="regi_login_input regi_login_input_left" placeholder="请输入收货人姓名" v-model="getName">
                  </li>
                  <li class="regi_form_input">
                    <i class="icon IconPeople"></i>
                    <input type="text" tabindex="2"  class="regi_login_input regi_login_input_left" placeholder="请输入收货地址" v-model="getAddress">
                  </li>
                  <li class="regi_form_input">
                    <i class="icon IconPeople"></i>
                    <input type="text" tabindex="3"  class="regi_login_input regi_login_input_left" placeholder="请输入联系电话" v-model="getPhone">
                  </li>
                  <li class="regi_form_input">
                    <i class="icon IconPeople"></i>
                    <input type="text" tabindex="4"  class="regi_login_input regi_login_input_left" placeholder="请输入邮编" v-model="getPost">
                  </li>
                </ul>
              </div>
              <div class="login-wrap">
                <a href="javascript:;" class="btn-login" @click="addGetAddress">添  加</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <modal :mdShow="isMdShow2" @close="isMdShow2=false">
        ...
        <div slot="btnGroup">
            <a class="btn btn--m btn--red" href="javascript:;" @click="isMdShow2=false">退出</a>
        </div>
      </modal>
    </div>
</template>
  ...
  export default{
      data(){
          return{
              ...
              errorTipsDisplay:false,
              rightTipsDisplay:false,
              errorTips:'',
              rightTips:'',
              getName:'',
              getAddress:'',
      ...
      methods: {
        init(){
          ...
          addGetAddress(){
              if(this.getName===''||this.getAddress===''||this.getPhone===''||this.getPost===''){
                this.errorTips='请填写完所有信息！'
                this.errorTipsDisplay=true
              }else{
                axios.post("/users/addAddress",{
                  userName:this.getName,
                  streetName:this.getAddress,
                  postCode:this.getPost,
                  tel:this.getPhone,
                }).then((response)=>{
                  let res = response.data;
                  if(res.status=="0"){
